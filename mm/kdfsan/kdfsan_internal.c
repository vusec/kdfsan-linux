...
static void kdf_print_bitvector(dfsan_label lbl) {
  ...
  kspecem_hook_memcpy(str_kdf_print_bitvector,NUM_LABELS+1); __memset(str_kdf_print_bitvector,0,NUM_LABELS + 1);
...
static dfsan_label kdf_create_next_label(u8 *b, const char *desc) {
  ...
  if(b == NULL) {
    ...
    kspecem_hook_memcpy((char*)this_bitvector->b,NUM_LABELS); __memcpy(this_bitvector->b, b, NUM_LABELS); // size could probably just be this_label; that'd be slightly faster
  }
...
dfsan_label kdf_union(dfsan_label l1, dfsan_label l2) {
  ...
  kspecem_hook_memcpy((char*)b_tmp_kdf_union,NUM_LABELS); __memset(b_tmp_kdf_union,0,NUM_LABELS);
...
/**** Internals relevant to kspecem/kasan/kdf-pf-checker/etc. ****/
