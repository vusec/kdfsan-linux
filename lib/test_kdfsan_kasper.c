...
void kdf_run_kasper_tests(void) {
  ...
  printk("    KDFSan: Running load test with tainted out-of-bounds index... (SHOULD PRINT 1 RIDL REPORT)\n");
  ...
  printk("    KDFSan: Running store test with tainted out-of-bounds index and untainted value... (SHOULD PRINT 1 RIDL REPORT)\n");
  BUF_ALLOC(tmp_buf); testkasper_store(tmp_buf,attck_outofbounds,clean_val); BUF_FREE(tmp_buf);
  printk("    KDFSan: Running store test with tainted out-of-bounds index and tainted value... (SHOULD PRINT 1 RIDL REPORT)\n");
  ...
  printk("    KDFSan: Running speculative load test with tainted out-of-bounds index... (SHOULD PRINT 1 RIDL REPORT)\n");
  ...
  printk("    KDFSan: Running speculative load test with tainted *global* out-of-bounds index... (SHOULD PRINT 1 RIDL REPORT)\n");
  testkasper_spec_load(glob_arr, ARR_SIZE, attck_outofbounds);

  // Multiple-independent-accesses tests
  printk("    KDFSan: Running multiple-independent-accesses test with 1 (first) tainted out-of-bounds index... (SHOULD PRINT 1 RIDL REPORT)\n");
  testkasper_independent_accesses(arr1,ARR_SIZE,attck_outofbounds,clean_inbounds);
  printk("    KDFSan: Running multiple-independent-accesses test with 1 (second) tainted out-of-bounds index... (SHOULD PRINT 1 RIDL REPORT)\n");
  testkasper_independent_accesses(arr1,ARR_SIZE,clean_inbounds,attck_outofbounds);
  printk("    KDFSan: Running multiple-independent-accesses test with 2 (both) tainted out-of-bounds indices... (SHOULD PRINT 2 RIDL REPORTS)\n");
  ...
  printk("    KDFSan: Running two-dependent-accesses test with untainted out-of-bounds index to slab object... (xxx + 1 SLAB-MASSAGE-READ RIDL REPORT)\n");
  ...
  printk("    KDFSan: Running two-dependent-accesses test with untainted out-of-bounds index to stack object... (xxx + 1 STACK-MASSAGE-READ RIDL REPORT)\n");
  ...
  printk("    KDFSan: Running two-dependent-accesses test with tainted *stack* out-of-bounds index... (SHOULD PRINT 1 RIDL REPORT + 1 SPECTREV1 REPORT)\n");
  testkasper_two_dependent_accesses_arr(arr1,arr2,ARR_SIZE,attck_outofbounds);
  printk("    KDFSan: Running two-dependent-accesses test with tainted *global* out-of-bounds index... (SHOULD PRINT 1 RIDL REPORT + 1 SPECTREV1 REPORT)\n");
  testkasper_two_dependent_accesses_arr(glob_arr,arr2,ARR_SIZE,attck_outofbounds);

  // Three-dependent-accesses tests
  printk("    KDFSan: Running three-dependent-accesses test with untainted *slab* out-of-bounds index... (xxx + 1 RIDL REPORT + 1 SPECTREV1 REPORT)\n");
  ...
  printk("    KDFSan: Running three-dependent-accesses test with tainted *slab* out-of-bounds index... (SHOULD PRINT 1 RIDL REPORT + 2 SPECTREV1 REPORTS)\n");
  ...
  printk("    KDFSan: Running new-label-in-spec test... (SHOULD PRINT 1 RIDL REPORT)\n");
  testkasper_newlabel_inspec(arr1, ARR_SIZE, attck_outofbounds);

  // Independent-page-faults-in-speculation test
  printk("    KDFSan: Running independent-page-fault test... (SHOULD PRINT 2 RIDL REPORTS)\n");
  ...
  printk("    KDFSan: Running *tainted wild-mem* #1 dependent-page-fault test... (SHOULD PRINT 1 RIDL REPORT + 1 SPECTREV1 REPORT)\n");
  testkasper_two_dependent_accesses_ptr((int**) tainted_wild1);
  printk("    KDFSan: Running *tainted wild-mem* #2 dependent-page-fault test... (SHOULD PRINT 1 RIDL REPORT + 1 SPECTREV1 REPORT)\n");
  testkasper_two_dependent_accesses_ptr((int**) tainted_wild2);
  printk("    KDFSan: Running *tainted user-mem* dependent-page-fault test... (SHOULD PRINT 1 RIDL REPORT + 1 SPECTREV1 REPORT)\n");
  testkasper_two_dependent_accesses_ptr((int**) tainted_user);
  printk("    KDFSan: Running *tainted null-mem* dependent-page-fault test... (SHOULD PRINT 1 SPECTREV1 REPORT)\n");
  testkasper_two_dependent_accesses_ptr((int**) tainted_null);
  ...
  printk("    KDFSan: Running *untainted wild-mem* #1 LVI test... (SHOULD PRINT 1 RIDL REPORT)\n");
  testkasper_two_dependent_accesses_ptr((int**) untainted_wild1);
  printk("    KDFSan: Running *untainted wild-mem* #2 LVI test... (SHOULD PRINT 1 RIDL REPORT)\n");
  testkasper_two_dependent_accesses_ptr((int**) untainted_wild2);
  printk("    KDFSan: Running *untainted user-mem* LVI test... (SHOULD PRINT 1 RIDL REPORT)\n");
  testkasper_two_dependent_accesses_ptr((int**) untainted_user);
  printk("    KDFSan: Running *untainted null-mem* LVI test...\n");
  testkasper_two_dependent_accesses_ptr((int**) untainted_null);
