...
static void test_usercopy_run(char *kmem, char __user *usermem, size_t size, char data, dfsan_label expected_label) {
  // Test copy_from_user
  clear_mem(kmem, size);
...
static void test_usercopy(void) {
  ...
  test_usercopy_run(kmem, usermem, size, data, usercopy_label); // usercopy output should only have the usercopy label
  ...
  ASSERT(dfsan_get_label(size) == 0 && dfsan_get_label(data) == 0);
  test_usercopy_run(kmem, usermem, size, data, unioned_label); // usercopy output should have both the usercopy label and the attacker label
...
void kdf_policies_run_tests(void) {
  test_usercopy();
